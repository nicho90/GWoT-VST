			...
			if(!validation.success) {
				...
				pg.connect(url, function(err, client, done) {
					if(err) {
						...
						// Database Query
						client.query('SELECT * FROM Subscriptions WHERE username=$1 AND subscription_id;', [
							req.params.username,
							...
							if(err) {
								...
								if(result.rows.length === 0) {
									...
									// Database Query
									client.query("DELETE FROM Subscriptions WHERE username=$1 AND subscription_id=$2;", [
										req.params.username,
										req.params.subscription_id
									], function(err, result) {
										done();
