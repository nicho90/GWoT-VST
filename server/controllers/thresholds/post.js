				...
				if (!valid) {
					...
					pg.connect(url, function(err, client, done) {
						if(err) {
							...
							// Database Query
							client.query("INSERT INTO Thresholds (created, updated, username, description, value, category) VALUES (now(), now(), $1, $2, $3, $4);",
								[
									req.params.username,
								...
								if(err) {
									...
									// Database Query
									client.query('SELECT * FROM thresholds WHERE username=$1 ORDER BY created DESC;', [
										req.params.username
									], function(err, result) {
										done();
